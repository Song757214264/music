require(["text!../music/music-list.json","jquery.min"],function(listData){
	...
	function loadMusic(){
		...
		$("#back").css("background-image","url(music/"+musicName+".jpg)");
	...
	function changeProgress(e){
		var progre = e.offsetX/e.target.offsetWidth;
		//把点击进度条计算之后的进度显示在进度条上
		$("#progress-bar").val(progre);
		//根据点击的进度设置播放器的当前播放进度
		$("#player")[0].currentTime = $("#player")[0].duration*progre;
	}
    ...
    $("#player")[0].ontimeupdate = function(e){
    	...
    	if(currentLine!=line||line==0){
    		
    		$("#lyric-box p")[currentLine].classList.remove("highlight-line");
	    	$("#lyric-box p")[line].classList.add("highlight-line");
	    	currentLine = line;
	    	console.log(line);
